...
import { ImageAltTextSettings, NewAltTextManager } from "./new_alt_text_manager";
...
export class WebPDFViewer {
  ...
  constructor(viewerContext: WebPDFViewerContext) {
...
const PDFViewerApplication = {
  ...
  _cleanup() {
    if (!this.pdfDocument) {
      return; // run cleanup when document is loadedfini
    }
...
initCom(WebPDFViewer);

if (typeof PDFJSDev === "undefined" || !PDFJSDev.test("MOZCENTRAL")) {
  PDFPrintServiceFactory.initGlobals(WebPDFViewer);
}
...
if (typeof PDFJSDev === "undefined" || PDFJSDev.test("GENERIC")) {
  ...
  var validateFileURL = function (file) {
    ...
    try {
      ...
      WebPDFViewer._documentError("pdfjs-loading-error", {
        message: ex.message,
      });
      throw ex;
    }
