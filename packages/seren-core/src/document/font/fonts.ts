...
function adjustTrueTypeToUnicode(properties: EvaluatorProperties
  , isSymbolicFont: boolean, nameRecords: NameRecordState[]) {
  ...
  for (const r of nameRecords) {
    if (!isWinNameRecord(r.platform, r.encoding, r.language)) {
...
function adjustType1ToUnicode(properties: EvaluatorProperties, builtInEncoding: (string | number)[]) {
  if (properties.isInternalFont) {
    return;
  }
  if (properties.hasIncludedToUnicodeMap) {
  ...
  for (const charCode in builtInEncoding) {
    ...
    const unicode = getUnicodeForGlyph(<string>glyphName, glyphsUnicodeMap);
    if (unicode !== -1) {
      // 因为是charCode in的缘故，这里做一个强转
      toUnicode[Number.parseInt(charCode)] = String.fromCharCode(unicode);
    }
  }
  if (toUnicode.length > 0) {
    (<ToUnicodeMapImpl | IdentityToUnicodeMapImpl>properties.toUnicode).amend(toUnicode);
...
function isTrueTypeFile(file: Stream) {
...
function isTrueTypeCollectionFile(file: Stream) {
  const header = file.peekBytes(4);
  return bytesToString(header) === "ttcf";
}

function isOpenTypeFile(file: Stream) {
  const header = file.peekBytes(4);
  return bytesToString(header) === "OTTO";
}

function isType1File(file: Stream) {
 ...
 */
function isCFFFile(file: Stream) {
  const header = file.peekBytes(4);
  if (
...
function getFontFileType(file: Stream, { type, subtype, composite }: EvaluatorProperties)
  : [string, string | null] {
  let fileType, fileSubtype = null;
...
function isMacNameRecord(platform: number, encoding: number, language: number) {
  return platform === 1 && encoding === 0 && language === 0;
}

// Please refer to:
//  - https://developer.apple.com/fonts/TrueType-Reference-Manual/RM06/Chap6name.html
//  - https://learn.microsoft.com/en-us/typography/opentype/spec/name#windows-language-ids
function isWinNameRecord(platform: number, encoding: number, language: number) {
  return platform === 3 && encoding === 1 && language === 0x409;
}
...
function adjustMapping(
  ...
  for (const _originalCharCode in charCodeToGlyphId) {
    const originalCharCode = Number.parseInt(_originalCharCode);
...
function getRanges(glyphs: Record<number, number>
  , toUnicodeExtraMap: Map<number, number>
  , numGlyphs: number): [number, number, number[]][] {
  // Array.sort() sorts by characters, not numerically, so convert to an
  // array of characters.
  const codes = [];
  for (const _charCode in glyphs) {
...
function createCmapTable(glyphs: Record<number, number>, toUnicodeExtraMap: Map<number, number>, numGlyphs: number) {
...
function validateOS2Table(os2: TablePropType<Uint8TypedArray>, file: Stream) {
  ...
  // 这里直接给string用下标来配置0，也许是bug，也许是我没看懂
  (<Uint8Array>os2.data)[8] = (<Uint8Array>os2.data)[9] = 0; // IE rejects fonts if fsType != 0
  return true;
}
...
function createOS2Table(
  ...
  if (charstrings) {
    for (let _code in charstrings) {
      let code = Number.parseInt(_code);
      code |= 0;
  ...
  return (
    ...
    string16(<number>firstCharIndex || properties.firstChar) + // usFirstCharIndex
    ...
    string16(<number>firstCharIndex || properties.firstChar) + // usBreakChar
    "\x00\x03"
  ); // usMaxContext
}
...
class Font {
  ...
  public lineGap: number | null = null;
  ...
  checkAndRepair(name: string, font: Stream, properties: EvaluatorProperties) {
    // 这里的类型必须要和ValidTableType对得上才行
    const VALID_TABLES = [
      "OS/2",
      "cmap",
      "head",
    ...
    function readTables(file: Stream, numTables: number): ValidTableInfo {
      ...
      for (let i = 0; i < numTables; i++) {
        ...
        tables[<ValidTableType>table.tag] = table;
      }
      return tables;
    }

    function readTableEntry(file: Stream) {
    ...
    function readTrueTypeCollectionHeader(ttc: Stream) {
      ...
      const header = {
        ...
        numFonts,
        offsetTable,
        dsigTag: <number | null>null,
        dsigLength: <number | null>null,
        dsigOffset: <number | null>null
      };
      switch (majorVersion) {
        case 1:
          return header;
    ...
    function readTrueTypeCollectionData(ttc: Stream, fontName: string) {
    ...
    function readCmapTable(
      ...
      let potentialTable: { platformId: number, encodingId: number, offset: number } | null = null;
      let canBreak = false;
      ...
      for (let i = 0; i < numTables; i++) {
        ...
        if (
          potentialTable && potentialTable.platformId === platformId &&
      ...
      if (format === 0) {
        ...
        const segments: { end?: number, start?: number, delta?: number, offsetIndex?: number }[] = [];
        let segIndex;
    ...
    function sanitizeGlyphLocations(
      ...
      if (isGlyphLocationsLong) {
        itemSize = 4;
        itemDecode = function fontItemDecodeLong(data: Uint8Array, offset: number) {
          return (
            (data[offset] << 24) |
            (data[offset + 1] << 16) |
            (data[offset + 2] << 8) |
    ...
    function readNameTable(nameTable: TablePropType<Uint8TypedArray | string>): [[string[], string[]], NameRecordState[]] {
      ...
      for (i = 0; i < numRecords && font.pos + NAME_RECORD_LENGTH <= end; i++) {
        ...
        if (
          isMacNameRecord(r.platform, r.encoding, r.language) ||
          isWinNameRecord(r.platform, r.encoding, r.language)) {
          records.push(r);
        }
      }
    ...
    function checkInvalidFunctions(ttContext: TTContext, maxFunctionDefs: number) {
    ...
    // 或许这里应该使用Uin8Array而不是string，还是得考虑一下
    tables.post = {
      tag: "post",
        ...
        for (let charCode = 0; charCode < 256; charCode++) {
          ...
          if (unicodeOrCharCode === undefined) {
            ...
            if (
              ...
              if (unicode) {
                unicodeOrCharCode = (<string>unicode).codePointAt(0);
              }
            }
    ...
    // 这里原来使用.length来记录长度，但是会报错，因为record没有length属性
    // 因此改用keys来解决了
    if (Object.keys(charCodeToGlyphId).length === 0) {
  ...
  convert(fontName: string, font: CFFFont | Type1Font, properties: EvaluatorProperties) {
    ...
    function getCharCodes(charCodeToGlyphId: number[] | Record<number, number>, glyphId: number) {
      let charCodes = null;
      for (const charCode in charCodeToGlyphId) {
        if (glyphId === charCodeToGlyphId[charCode]) {
          // 因为forin的问题，这里的charcode强转了一下
          (charCodes ||= []).push(Number.parseInt(charCode) | 0);
        }
      }
      return charCodes;
    }

    function createCharCode(charCodeToGlyphId: number[] | Record<number, number>, glyphId: number) {
      for (const charCode in charCodeToGlyphId) {
        if (glyphId === charCodeToGlyphId[charCode]) {
          // 因为forin的问题，这里的charcode强转了一下
          return Number.parseInt(charCode) | 0;
        }
      }
    ...
    builder.addTable(
      ...
      (function fontFieldsHmtx() {
        const charstrings = (<Type1Font>font).charstrings;
        const cffWidths = (<CFFFont>font).cff ? (<CFFFont>font).cff.widths : null;
  ...
  _charToGlyph(charcode: number, isSpace = false) {
    ...
    let fontCharCode: number;
    let width, operatorListId;
